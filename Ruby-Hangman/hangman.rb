module Hangman
  ...
  def self.choose_word_from_file(min_word_length, max_word_length)
    filepath = File.join(__dir__, 'wordlist.txt')
    wordfile = File.open(filepath, 'r')
    word_list = []
  ...
  def self.valid_guess_input?(input_string, hidden_word, letter_guesses)
  ...
  def self.accept_guess_input(prompt_string, hidden_word, letter_guesses)
    loop do
      puts prompt_string
  ...
  def self.hidden_word_progress(letter_guesses, hidden_word)
  ...
  def self.letter_guess_feedback(letter_guess, letter_guesses, hidden_word, wrong_guesses)
  ...
  def self.draw_hangman(wrong_guesses)
    [
    '    +---+',
    '    |   |',
    "    #{wrong_guesses >= 1 ? 'O' : ' '}   |",
